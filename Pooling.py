...
class Pooling(Base.BaseLayer): # very important layer from spatial invariance perspective if we want our CNN model to
                               # recognize an object in an image correctly even if that object hass its features tilted, distorted
                               # different light composition, the texture so it explains this concept using visual representation
    ...
    def forward(self, input_tensor):
        ...
        for b in range(batches):
            ...
            for c in range(channels): 
                ...
                # How much the pooling filter fits  in every image
                # pooling_shape[0] for x dimension
                # pooling_shape[1] for y dimension
                if self.pooling_shape[0]%2 == 0: w = int(image_part.shape[0]-np.ceil((self.pooling_shape[0]-1)/2)) # np. ceil() function in Python is used to find the ceil of the elements of the array.
